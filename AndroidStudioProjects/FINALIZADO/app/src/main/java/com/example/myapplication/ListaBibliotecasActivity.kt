                ...
                onItemClick = { biblioteca ->
                    ...
                    if (action == "edit") {
                        val intent = Intent(this, Biblioteca::class.java).apply {
                            putExtra("msg", "edit")  // se lo pasamos a la Activity Biblioteca
                    ...
                    // Si la acción original era para editar (o eliminar) libros, navegamos a la lista de libros
                    else {
                        val bId = biblioteca["id"] as Int
                        // Verificamos si hay libros para esa biblioteca
                        val cursor = db?.rawQuery(
                        ...
                        cursor?.use {
                            if (it.moveToFirst() && it.getInt(0) > 0) {
                                // Vamos a la lista de libros, pasándole la misma acción que nos llegó ("delete" o "edit")
